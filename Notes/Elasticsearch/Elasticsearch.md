- [elasticsearch 的安装](#elasticsearch-的安装)
  - [Linux 环境](#linux-环境)
  - [Windows 环境](#windows-环境)
  - [Mac 环境](#mac-环境)
- [Elasticsearch 入门](#elasticsearch-入门)
  - [简介](#简介)
  - [相关概念](#相关概念)
  - [集群状态查看](#集群状态查看)
  - [索引操作](#索引操作)
...
- [Spring data Elasticsearch](#spring-data-elasticsearch)
  - [常用注解](#常用注解)
  - [Spring data 方式的数据操作](#spring-data-方式的数据操作)
    - [在pom.xml中添加相关依赖](#在pomxml中添加相关依赖)
    - [修改SpringBoot配置文件](#修改springboot配置文件)
    - [添加商品文档对象EsProduct](#添加商品文档对象esproduct)
    - [添加EsProductRepository接口用于操作Elasticsearch](#添加esproductrepository接口用于操作elasticsearch)
    - [添加EsProductService接口](#添加esproductservice接口)
    - [添加EsProductService接口的实现类EsProductServiceImpl](#添加esproductservice接口的实现类esproductserviceimpl)
    - [添加EsProductController定义接口](#添加esproductcontroller定义接口)
...
​	一个基于 Lucene 的搜索服务器。提供了一个分布式的搜索引擎，基于 RESTful web 接口。

​	采用 Java 语言开发，基于 Apache 协议的开源项目。

## 相关概念

+ **Near Realtime**（近实时）：Elasticsearch 是一个近乎实时的搜索平台，这意味着从索引文档到可搜索文档之间只有一个轻微的延迟。
+ **Cluster**（集群）：一个或多个节点的集合，一起保存整个数据，并提供跨所有节点的联合索引和搜索功能。每个集群，唯一的集群名称，节点通过名称加入集群。
...
+ **Type**（类型）：类型是索引的逻辑类别分区，通常，为具有一组公共字段的文档类型，类似MySql中表的概念。<u>注意</u>：在Elasticsearch 6.0.0及更高的版本中，一个索引只能包含一个类型。
+ **Document**（文档）：文档是可被索引的基本信息单位，以JSON形式表示，类似于MySql中行记录的概念。
+ **Shards**（分片）：当索引存储大量数据时，可能会超出单个节点的硬件限制，为了解决这个问题，Elasticsearch提供了将索引细分为分片的概念。**分片机制**赋予了索引水平扩容的能力、并允许跨分片**分发和并行化**操作，从而提高性能和吞吐量。
...
{"delete":{"_index":"movies", "_type":"movie", "_id":2}}
...
查询表达式（Query DSL），使用它可以以简单的JSON接口来实现丰富的搜索功能
...
# 组合搜索，bool 表示，must——同时满足，should——满足其中任意一个，must_not——同时不满足
GET /bank/_search
{
  "query": {
    "bool": {
